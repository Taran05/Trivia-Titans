...
/**
 * This function handles the retrieval of user statistics from the DynamoDB table based on the user's id obtained from users table.
...
const handler = async (event) => {
    try {
        ...
        // Prepare the QueryCommand to fetch user details from DynamoDB based on the email
        ...
        // Send the QueryCommand to DynamoDB to retrieve user details by email
        const result = await dynamoDocClient.send(getCommand);
        console.log(result.Items[0].id);
        
        const userId = result.Items[0].id

        // Prepare the QueryCommand to fetch user statistics from DynamoDB based on the userId
        ...
        // Send the QueryCommand to DynamoDB to retrieve user stats by userId
